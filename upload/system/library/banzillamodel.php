...
class BanzillaModel extends Model {
    ...
    public function addOrder($data) {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "banzilla_order SET order_id = " . (int) $data['order_id'] . ", charge_ref = '" . $this->db->escape($data['charge_ref']) . "', date_added = NOW(), capture_status = " . (int) $data['capture_status'] . ", description  = '" . $this->db->escape($data['description']) . "', total = '" . (float) $data['total'] . "', currency_code = '" . $this->db->escape($data['currency_code']) . "'");
    ...
    public function getOrder($order_id) {
        ...
        $order = $this->db->query("SELECT * FROM " . DB_PREFIX . "banzilla_order WHERE order_id = '" . (int) $order_id . "'");
        if ($order->num_rows) {
    ...
    public function getOrderByCharge($charge_ref) {
        ...
        $order = $this->db->query("SELECT * FROM " . DB_PREFIX . "banzilla_order WHERE charge_ref = '" . $this->db->escape($charge_ref) . "'");
        if ($order->num_rows) {
    ...
    public function getTransactions($data) {
        $q = "SELECT * FROM " . DB_PREFIX . "banzilla_transaction";
        $qa = array();
    ...
    public function addCustomer($data) {
        $info = array(
            ...
            'banzilla_customer_id' => isset($data['banzilla_customer_id']) ? $data['banzilla_customer_id'] : ''
        );
        $this->debugLog->write("Record customer", $info);
        $this->db->query("INSERT INTO " . DB_PREFIX . "banzilla_customer SET customer_id = '" . $this->db->escape($info['customer_id']) . "', banzilla_customer_id = '" . $info['banzilla_customer_id'] . "', date_added = NOW()");
        if ($this->db->countAffected()) {
    ...
    public function getCustomer($customer_id) {
        //echo 'entre a modelo'; exit();
        $customer = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE customer_id = '" . $this->db->escape($customer_id) . "'");
        //echo '<pre>'; print_r ($customer->num_rows); echo '</pre>';exit();
